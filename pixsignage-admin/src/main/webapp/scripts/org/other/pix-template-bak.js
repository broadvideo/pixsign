...
function refreshTemplatezone(templatezone) {
	...
	if (templatezone.type == 0) {
		...
		if (templatezone.content != '' && templatezone.content != 'no' && templatezone.content != 'non') {
			$(templatezoneDiv).find('img').attr('src', '/pixsigdata' + templatezone.content);
...
var template_select_option = {
	...
	formatResult: function(data) {
		...
		var html = '<span><img src="/pixsigdata' + data.template.snapshot + '" width="' + width + 'px" height="' + height + 'px"/> ' + data.template.name + '</span>'
		return html;
	},
	formatSelection: function(data) {
		var width = 30;
