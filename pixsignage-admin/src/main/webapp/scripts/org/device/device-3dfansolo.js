	...
	var initDeviceTable = function () {
		var oTable = $('#DeviceTable').dataTable({
			...
			'aoColumns' : [ {'sTitle' : '', 'mData' : 'deviceid', 'bSortable' : false, 'sWidth' : '5%' }, 
							...
							{'sTitle' : common.view.playlist, 'mData' : 'terminalid', 'bSortable' : false, 'sWidth' : '15%' }, 
			...
			'fnRowCallback' : function(nRow, aData, iDisplayIndex) {
				...
				if (aData.defaultmedialist != null) {
					$('td:eq(5)', nRow).html(aData.defaultmedialist.name);
				} else {
					$('td:eq(5)', nRow).html('');
				}
				...
				if (aData.status == 1) {
					...
					buttonhtml += '<a href="javascript:;" data-id="' + iDisplayIndex + '" class="btn default btn-xs blue pix-medialist"><i class="fa fa-list-ul"></i> ' + common.view.playlist + ' </a>';
