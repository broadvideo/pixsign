...
$('#MyTable').dataTable({
	...
	'aoColumns' : [ {'sTitle' : common.view.name, 'mData' : 'name', 'bSortable' : false, 'sWidth' : '10%' }, 
					...
					{'sTitle' : common.view.schedule, 'mData' : 'devicegroupid', 'bSortable' : false, 'sWidth' : '65%' }, 
...
function refreshScheduleDetail() {
	...
	if (currentSchedules.length > 0) {
		...
		var scheduleTabHtml = '<h3>' + common.tips.devicegp_schedule_zero + '</h3>';
	}
...
$('body').on('click', '.pix-schedule', function(event) {
	...
	currentSchedules = currentDevicegroup.schedules;
...
$('#ScheduleForm .pix-ok').on('click', function(event) {
	if ($('#ScheduleForm').valid()) {
		...
		if (schedules.length > 0) {
			...
			schedule.bindtype = 2;
			schedule.bindid = currentDevicegroupid;
...
$('[type=submit]', $('#ScheduleModal')).on('click', function(event) {
	...
	data.scheduletype = 1;
	data.bindtype = 2;
	data.bindid = currentDevicegroupid;
