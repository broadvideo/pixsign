...
$('#MyTable').dataTable({
	...
	'aoColumns' : [ {'sTitle' : common.view.name, 'mData' : 'name', 'bSortable' : false, 'sWidth' : '10%' }, 
					...
					//{'sTitle' : common.view.layoutschedule, 'mData' : 'devicegroupid', 'bSortable' : false, 'sWidth' : '10%' }, 
					//{'sTitle' : common.view.regionschedule, 'mData' : 'devicegroupid', 'bSortable' : false, 'sWidth' : '10%' }, 
					{'sTitle' : common.view.bundleschedule, 'mData' : 'devicegroupid', 'bSortable' : false, 'sWidth' : '10%' }, 
					{'sTitle' : common.view.sync, 'mData' : 'devicegroupid', 'bSortable' : false, 'sWidth' : '5%' }],
	'iDisplayLength' : 10,
	...
	'fnRowCallback' : function(nRow, aData, iDisplayIndex) {
		...
		//$('td:eq(2)', nRow).html('<a href="javascript:;" privilegeid="101010" data-id="' + iDisplayIndex + '" class="btn default btn-xs blue pix-layoutschedule"><i class="fa fa-calendar-o"></i> ' + common.view.layoutschedule + '</a>');
		//$('td:eq(3)', nRow).html('<a href="javascript:;" privilegeid="101010" data-id="' + iDisplayIndex + '" class="btn default btn-xs blue pix-regionschedule"><i class="fa fa-calendar"></i> ' + common.view.regionschedule + '</a>');
		$('td:eq(2)', nRow).html('<a href="javascript:;" privilegeid="101010" data-id="' + iDisplayIndex + '" class="btn default btn-xs blue pix-bundleschedule"><i class="fa fa-calendar-o"></i> ' + common.view.bundleschedule + '</a>');
		$('td:eq(3)', nRow).html('<a href="javascript:;" privilegeid="101010" data-id="' + iDisplayIndex + '" class="btn default btn-xs green pix-sync"><i class="fa fa-rss"></i> ' + common.view.sync + '</a>');
...
function refreshLayoutScheduleDetail() {
	...
	if (currentLayoutschedules.length > 0) {
		...
		var scheduleTabHtml = '<h3>' + common.tips.devicegp_layoutschedule_zero + '</h3>';
	}
...
$('#LayoutScheduleForm .pix-ok').on('click', function(event) {
	if ($('#LayoutScheduleForm').valid()) {
		...
		if (layoutschedules.length > 0) {
			...
			layoutschedule.bindtype = 2;
			layoutschedule.bindid = currentDevicegroupid;
...
$('[type=submit]', $('#LayoutScheduleModal')).on('click', function(event) {
	...
	$.ajax({
		...
		data : '{"devicegroups":[' + $.toJSON(currentDevicegroup) + '], "layoutschedules":' + $.toJSON(currentLayoutschedules) + '}',
		dataType : 'json',
		contentType : 'application/json;charset=utf-8',
		beforeSend: function ( xhr ) {
			Metronic.startPageLoading({animate: true});
...
$('#RegionScheduleForm .pix-ok').on('click', function(event) {
	if ($('#RegionScheduleForm').valid()) {
		...
		if (regionschedules.length > 0) {
			...
			regionschedule.bindtype = 2;
			regionschedule.bindid = currentDevicegroupid;
...
$('[type=submit]', $('#RegionScheduleModal')).on('click', function(event) {
	...
	$.ajax({
		...
		data : '{"devicegroups":[' + $.toJSON(currentDevicegroup) + '], "regionschedules":' + $.toJSON(currentRegionschedules) + '}',
		dataType : 'json',
		contentType : 'application/json;charset=utf-8',
		beforeSend: function ( xhr ) {
			Metronic.startPageLoading({animate: true});
...
function refreshBundleScheduleDetail() {
	...
	if (currentBundleschedules.length > 0) {
		...
		var scheduleTabHtml = '<h3>' + common.tips.devicegp_bundleschedule_zero + '</h3>';
	}
...
$('#BundleScheduleForm .pix-ok').on('click', function(event) {
	if ($('#BundleScheduleForm').valid()) {
		...
		if (bundleschedules.length > 0) {
			...
			bundleschedule.bindtype = 2;
			bundleschedule.bindid = currentDevicegroupid;
...
$('[type=submit]', $('#BundleScheduleModal')).on('click', function(event) {
	...
	$.ajax({
		...
		data : '{"devicegroups":[' + $.toJSON(currentDevicegroup) + '], "bundleschedules":' + $.toJSON(currentBundleschedules) + '}',
		dataType : 'json',
		contentType : 'application/json;charset=utf-8',
		beforeSend: function ( xhr ) {
			Metronic.startPageLoading({animate: true});
