...
import com.broadvideo.pixsign.persistence.DevicefilehisMapper;
...
import com.broadvideo.pixsign.service.DevicefileService;
import com.broadvideo.pixsign.service.WeatherService;
import com.broadvideo.pixsign.util.CommonUtil;
import com.broadvideo.pixsign.util.ipparse.IPSeeker;
			...
			if (device.getBackupvideo() != null) {
				...
				backupvideoJson.put("url",
						"http://" + configMapper.selectValueByCode("ServerIP") + ":"
								+ configMapper.selectValueByCode("ServerPort") + "/pixsigndata"
								+ device.getBackupvideo().getFilepath());
				backupvideoJson.put("path", "/pixsigndata" + device.getBackupvideo().getFilepath());
				...
				backupvideoJson.put("path", "/pixsigndata" + org.getBackupvideo().getFilepath());
				...
				if (defaultOrg.getBackupvideo() != null) {
					...
					backupvideoJson.put("path", "/pixsigndata" + defaultOrg.getBackupvideo().getFilepath());
		...
		try {
			...
			if (device.getUpgradeflag().equals("0")) {
				...
				if (appfile == null) {
					...
					String url = downloadurl + "/pixsigndata" + appfile.getFilepath();
					String path = "/pixsigndata" + appfile.getFilepath();
