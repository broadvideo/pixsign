...
function generateBundledtlHtml(templetdtl) {
	...
	if (templetdtl.bgimage != null) {
		templetdtlhtml += '<img src="/pixsigndata' + templetdtl.bgimage.thumbnail+ '" width="100%" height="100%" style="position: absolute; right: 0; bottom: 0; top: 0; left: 0; z-index: 0" />';
	}
	...
	$('#SubBundleSelect').select2({
		...
		formatResult: function (item) {
			if (item.templet != null && item.templet.snapshot != null) {
				return '<span><img src="/pixsigndata' + item.templet.snapshot + '" height="25" /> ' + item.name + '</span>';
			} else {
