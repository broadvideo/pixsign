...
function redrawLayoutPreview(div, layout, maxsize) {
	...
	if (layout.bgimage != null) {
		div.append('<img class="layout-bg" src="/pixsigndata' + layout.bgimage.thumbnail + '" width="100%" height="100%" style="right: 0; bottom: 0; position: absolute; top: 0; left: 0; z-index: 0" />');
	}
...
function getLayoutdtlPreviewHtml(layout, layoutdtlindex) {
	...
	if (layoutdtl.bgimage != null) {
		bgimage = '/pixsigndata' + layoutdtl.bgimage.thumbnail;
...
function redrawLayoutCanvasPreview(canvas, layout) {
	...
	if (layout.bgimage != null) {
		...
		layout_bgimage.src = '/pixsigndata' + layout.bgimage.thumbnail;
...
function drawCanvasRegion(ctx, bundle, layoutdtl, left, top, width, height, fill) {
	...
	if (bgimage != null) {
		...
		var region_bgimage = new Image();
		region_bgimage.src = '/pixsigndata' + layoutdtl.bgimage.thumbnail;
		region_bgimage.onload = function(img, ctx, left, top, width, height) {
			return function() {
				ctx.drawImage(img, left, top, width, height);
			}
...
function redrawTempletPreview(div, templet, maxsize, dynamic) {
	...
	if (templet.bgimage != null) {
		div.append('<img class="layout-bg" src="/pixsigndata' + templet.bgimage.thumbnail + '" width="100%" height="100%" style="right: 0; bottom: 0; position: absolute; top: 0; left: 0; z-index: 0" />');
	}
...
function getTempletdtlPreviewHtml(templet, templetdtlindex, dynamic) {
	...
	if (templetdtl.bgimage != null) {
		bgimage = '/pixsigndata' + templetdtl.bgimage.thumbnail;
...
function redrawMediagridPreview(div, mediagrid, maxsize) {
	...
	for (var i=0; i<mediagrid.mediagriddtls.length; i++) {
		...
		if (mediagriddtl.video != null) {
			bgimage = '/pixsigndata' + mediagriddtl.video.thumbnail;
		} else if (mediagriddtl.image != null) {
			bgimage = '/pixsigndata' + mediagriddtl.image.thumbnail;
		} else if (mediagriddtl.page != null) {
			bgimage = '/pixsigndata' + mediagriddtl.page.snapshot;
		} else if (mediagriddtl.bundle != null) {
			bgimage = '/pixsigndata' + mediagriddtl.bundle.snapshot;
		}
	...
	for (var i = 0; i < zones.length; i++) {
		...
		if (zone.type == 1) {
			...
			if (zonedtls.length > 0 && zonedtls[0].image != null) {
				$(zone_div).find('img').attr('src', '/pixsigndata' + zonedtls[0].image.filepath);
				$(zone_div).find('img').attr('width', '100%');
				$(zone_div).find('img').attr('height', '100%');
			}
		} else if (zone.type == '3') {
			...
			if (zonedtls.length > 0 && zonedtls[0].image != null) {
				$(inner_div).css('background-image', 'url(/pixsigndata' + zonedtls[0].image.filepath + ')');
			...
			if (zone.diy != null) {
				$(zone_div).find('img').attr('src', '/pixsigndata' + zone.diy.thumbnail);
				$(zone_div).find('img').attr('width', '100%');
				$(zone_div).find('img').attr('height', '100%');
			}
		} else {
