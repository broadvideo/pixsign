...
function redrawBundle(div, bundle, bundledtl) {
	...
	if (bundle.layout.bgimage != null) {
		div.append('<img class="layout-bg" src="/pixsigndata' + bundle.layout.bgimage.thumbnail + '" width="100%" height="100%" style="right: 0; bottom: 0; position: absolute; top: 0; left: 0; z-index: 0" />');
	}
...
function redrawBundledtl(div, bundle, bundledtl, selected) {
	...
	if (bundledtl.layoutdtl.bgimage != null) {
		bgimage = '/pixsigndata' + bundledtl.layoutdtl.bgimage.thumbnail;
	...
	var text = "";
	if (bundledtl.objtype == 2 && bundledtl.type == 0) {
...
function generateBundledtlHtml(bundledtl) {
	...
	if (bundledtl.layoutdtl.bgimage != null) {
		bundledtlhtml += '<img src="/pixsigndata' + bundledtl.layoutdtl.bgimage.thumbnail+ '" width="100%" height="100%" style="position: absolute; right: 0; bottom: 0; top: 0; left: 0; z-index: 0" />';
	}
				...
				for (var i=0; i<item.medialist.medialistdtls.length; i++) {
					...
					if (medialistdtl.objtype == 1) {
						if (medialistdtl.video.thumbnail == null) {
							...
							html += '<img src="/pixsigndata' + medialistdtl.video.thumbnail + '" alt="' + medialistdtl.video.name + '" width="100%" />';
						}
					} else if (medialistdtl.objtype == 2) {
						html += '<img src="/pixsigndata' + medialistdtl.image.thumbnail + '" alt="' + medialistdtl.image.name + '" width="100%" />';
					}
					html += '</div>';
				}						
				html += '</div>';
	...
	$('#SubBundleSelect').select2({
		...
		formatResult: function (item) {
			if (item.bundle != null && item.bundle.snapshot != null) {
				return '<span><img src="/pixsigndata' + item.bundle.snapshot + '" height="25" /> ' + item.name + '</span>';
			} else {
	...
	'fnRowCallback': function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
		...
		if (aData.relate != null) {
			...
			if (aData.relate.thumbnail != null) {
				thumbnail = '/pixsigndata' + aData.relate.thumbnail;
...
$('#ImageTable').dataTable({
	...
	'fnRowCallback': function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
		...
		if (aData.relate != null) {
			...
			imagehtml += '<img src="/pixsigndata' + aData.relate.thumbnail + '" width="100%" alt="' + aData.relate.name + '" thumbwidth="' + thumbwidth + '" thumbheight="' + thumbheight + '"/>';
			imagehtml += '</div>';
		}
