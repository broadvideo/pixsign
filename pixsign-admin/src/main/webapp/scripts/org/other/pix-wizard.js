...
function submitData() {
	...
	$('#ScheduleTable').dataTable({
		...
		'aoColumns' : [ {'sTitle' : '终端', 'mData' : 'device.name', 'bSortable' : false }, 
						...
						{'sTitle' : '已下�?', 'mData' : 'device.deviceid', 'bSortable' : false }, 
						{'sTitle' : '待下�?', 'mData' : 'device.deviceid', 'bSortable' : false }, 
						{'sTitle' : '开始下载时�?', 'mData' : 'device.deviceid', 'bSortable' : false }, 
						{'sTitle' : '剩余估算时间', 'mData' : 'device.deviceid', 'bSortable' : false }, 
						{'sTitle' : '同步', 'mData' : 'syncstatus', 'bSortable' : false }, 
						{'sTitle' : '状�?', 'mData' : 'status', 'bSortable' : false }],
		'iDisplayLength' : 10,
		'sPaginationType' : 'bootstrap',
		'oLanguage' : DataTableLanguage,
		'fnRowCallback' : function(nRow, aData, iDisplayIndex) {
			...
			if (aData['syncstatus'] == 0) {
				$('td:eq(7)', nRow).html('<span class="label label-sm label-warning">待同�?</span>');
			} else if (aData['syncstatus'] == 1) {
				$('td:eq(7)', nRow).html('<span class="label label-sm label-info">同步�?</span>');
			} else if (aData['syncstatus'] == 2) {
				$('td:eq(7)', nRow).html('<span class="label label-sm label-success">已同�?</span>');
			}
			if (aData['status'] == 0) {
	...
	$('#SchedulefileTable').dataTable({
		...
		'aoColumns' : [ {'sTitle' : '类型', 'mData' : 'filetype', 'bSortable' : false }, 
						...
						{'sTitle' : '文件�?', 'mData' : 'filename', 'bSortable' : false }, 
