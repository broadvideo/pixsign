var DeviceModule = function () {
	...
	var initScreenModal = function () {
		...
		$('#ScreenTable').dataTable({
			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
				...
				screenhtml += '<a href="/pixsigndata' + aData.screen + '" class="fancybox">';
				screenhtml += '<div class="thumbs">';
				screenhtml += '<img src="/pixsigndata' + aData.screen + '" class="imgthumb" width="' + 100 + '%" />';
				screenhtml += '</div>';
				screenhtml += '</a>';
	...
	var initDeviceFileModal = function () {
		...
		$('#DeviceVideoTable').dataTable({
			...
			'fnRowCallback' : function(nRow, aData, iDisplayIndex) {
				if (aData.video != null) {
					$('td:eq(1)', nRow).html('<img src="/pixsigndata' + aData.video.thumbnail + '" width="40px"></img>');
		...
		$('#DeviceImageTable').dataTable({
			...
			'fnRowCallback' : function(nRow, aData, iDisplayIndex) {
				if (aData.image != null) {
					$('td:eq(1)', nRow).html('<img src="/pixsigndata' + aData.image.thumbnail + '" width="40px"></img>');
		...
		$('#GridVideoTable').dataTable({
			...
			'fnRowCallback' : function(nRow, aData, iDisplayIndex) {
				if (aData.mmediadtl != null && aData.mmediadtl.video != null) {
					$('td:eq(1)', nRow).html('<img src="/pixsigndata' + aData.mmediadtl.video.thumbnail + '" width="40px"></img>');
		...
		$('#GridImageTable').dataTable({
			...
			'fnRowCallback' : function(nRow, aData, iDisplayIndex) {
				if (aData.mmediadtl != null && aData.mmediadtl.image != null) {
					$('td:eq(1)', nRow).html('<img src="/pixsigndata' + aData.mmediadtl.image.thumbnail + '" width="40px"></img>');
					$('td:eq(2)', nRow).html(aData.mmediadtl.filename);
					$('td:eq(3)', nRow).html(PixData.transferIntToComma(aData.mmediadtl.size));
				} else {
					$('td:eq(1)', nRow).html('');
