				...
				if (aData.plandtls.length > 0) {
					for (var i=0; i<aData.plandtls.length; i++) {
						...
						if (plandtl.objtype == 1) {
							...
							planhtml += '<img src="/pixsigndata' + plandtl.bundle.snapshot + '?t=' + timestamp + '" class="imgthumb" width="' + thumbwidth + '%" />';
							...
							planhtml += '<img src="/pixsigndata' + plandtl.page.snapshot + '" class="imgthumb" width="' + thumbwidth + '%" />';
						}
						planhtml += '</div>';
						planhtml += '</a>';
		...
		if (_plantype == 1) {
			...
			$('#PageTable').dataTable({
				...
				'fnRowCallback': function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
					...
					if (aData.snapshot != null) {
						var thumbwidth = aData.width > aData.height? 100 : 100*aData.width/aData.height;
						pagehtml += '<img src="/pixsigndata' + aData.snapshot + '?t=' + timestamp + '" class="imgthumb" width="' + thumbwidth + '%" alt="' + aData.name + '" />';
					} else {
			...
			$('#TouchpageTable').dataTable({
				...
				'fnRowCallback': function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
					...
					if (aData.snapshot != null) {
						var thumbwidth = aData.width > aData.height? 100 : 100*aData.width/aData.height;
						touchpagehtml += '<img src="/pixsigndata' + aData.snapshot + '?t=' + timestamp + '" class="imgthumb" width="' + thumbwidth + '%" alt="' + aData.name + '" />';
					} else {
			...
			for (var i=0; i<CurrentPlandtls.length; i++) {
				...
				if (plandtl.objtype == 1) {
					if (plandtl.bundle.snapshot != null) {
						thumbwidth = plandtl.bundle.width > plandtl.bundle.height? 100 : 100*plandtl.bundle.width/plandtl.bundle.height;
						thumbnail = '/pixsigndata' + plandtl.bundle.snapshot + '?t=' + timestamp;
					} else {
					...
					if (plandtl.page.snapshot != null) {
						thumbwidth = plandtl.page.width > plandtl.page.height? 100 : 100*plandtl.page.width/plandtl.page.height;
						thumbnail = '/pixsigndata' + plandtl.page.snapshot + '?t=' + timestamp;
					} else {
						thumbnail = '/pixsignage/img/blank.png';
					}
					mediatype = common.view.solopage;
