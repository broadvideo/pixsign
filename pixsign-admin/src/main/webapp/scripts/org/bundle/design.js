var BundleDesignModule = function (mode) {
	...
	var refreshBundlezone = function(bundlezone) {
		...
		if (bundlezone.bgimage != null) {
			$(bundlezoneDiv).find('#background').css({'background-image': 'url(/pixsigndata' + bundlezone.bgimage.thumbnail + ')' });
		} else {
		...
		if (bundlezone.type == 0) {
			...
			if (bundlezone.bundlezonedtls.length > 0 && bundlezone.bundlezonedtls[0].video != null) {
				$(bundlezoneDiv).find('img').attr('src', '/pixsigndata' + bundlezone.bundlezonedtls[0].video.thumbnail);
				...
				$(bundlezoneDiv).find('img').attr('src', '/pixsigndata' + bundlezone.bundlezonedtls[0].image.thumbnail);
				$(bundlezoneDiv).find('img').attr('width', '100%');
				$(bundlezoneDiv).find('img').attr('height', '100%');
			}
		} else if (bundlezone.type == 2) {
			...
			if (bundlezone.width > bundlezone.height) {
				...
				for (var i=0; i<bundlezone.bundlezonedtls.length; i++) {
					...
					var bgurl = '/pixsigndata' + bundlezone.bundlezonedtls[i].image.thumbnail;
					$(menu_div).css({
						'position': 'absolute',
						'height': '100%', 
						'width': menupercent + '%',
			...
			if (bundlezone.bundlezonedtls.length > 0 && bundlezone.bundlezonedtls[0].page != null) {
				$(bundlezoneDiv).find('img').attr('src', '/pixsigndata' + bundlezone.bundlezonedtls[0].page.snapshot);
				$(bundlezoneDiv).find('img').attr('width', '100%');
				$(bundlezoneDiv).find('img').attr('height', '100%');
			}
		} else if (bundlezone.type == 101 || bundlezone.type == 102 || bundlezone.type == 103 || bundlezone.type == 104) {
						...
						if (bundlezones[0].type == 1) {
							...
							$('#PageSelect').select2({
								...
								formatResult: function (item) {
									...
									var html = '<span><img src="/pixsigndata' + item.page.snapshot + '" width="' + width + 'px" height="' + height + 'px"/> ' + item.page.name + '</span>'
									return html;
								},
								formatSelection: function (item) {
									return item.name;				
			...
			for (var i=0; i<_self.Zone.bundlezonedtls.length; i++) {
				...
				if (bundlezonedtl.objtype == 1) {
					...
					if (bundlezonedtl.video.thumbnail == null) {
						...
						thumbnail = '/pixsigndata' + bundlezonedtl.video.thumbnail;
					}
					...
					thumbnail = '/pixsigndata' + bundlezonedtl.image.thumbnail;
