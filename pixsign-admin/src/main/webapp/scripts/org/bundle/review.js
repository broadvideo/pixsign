			...
			for (var i = 0; i < zones.length; i++) {
				...
				if (zone.type == 1) {
					...
					if (zonedtls.length > 0 && zonedtls[0].page != null) {
						$(zone_div).find('img').attr('src', '/pixsigndata' + zonedtls[0].page.snapshot);
						$(zone_div).find('img').attr('width', '100%');
						$(zone_div).find('img').attr('height', '100%');
					}
				...
				if (_bundlezone.bundlezonedtls != null) {
					for (var i=0; i<_bundlezone.bundlezonedtls.length; i++) {
						...
						if (bundlezonedtl.objtype == 1) {
							...
							thumbhtml += '<a href="/pixsigndata/video/preview/' + bundlezonedtl.video.videoid + '.mp4" class="fancybox">';
