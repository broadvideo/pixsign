var PageDesignModule = function (mode) {
	...
	var initEvent = function () {
		...
		function createZone(pagezone) {
			...
			} else if (pagezone.type == 10 || pagezone.type == 21 || pagezone.type == 31 || pagezone.type == 41) {
		...
		function refreshPagezone(pagezone) {
			...
			if (pagezone.type == 1) {
				...
				if (pagezone.pagezonedtls.length > 0 && pagezone.pagezonedtls[0].video != null) {
					$(pagezoneDiv).find('img').attr('src', '/pixsigndata' + pagezone.pagezonedtls[0].video.thumbnail);
				...
				if (pagezone.pagezonedtls.length > 0 && pagezone.pagezonedtls[0].image != null) {
					$(pagezoneDiv).find('img').attr('src', '/pixsigndata' + pagezone.pagezonedtls[0].image.thumbnail);
					$(pagezoneDiv).find('img').attr('width', '100%');
					$(pagezoneDiv).find('img').attr('height', '100%');
				}
			} else if (pagezone.type == 3) {
				...
				$(pagezoneDiv).find('marquee').attr('scrollamount', pagezone.speed);
				var text_val = pagezone.content;
				if (text_val == undefined || text_val == null || text_val == '') {
				...
				if (pagezone.pagezonedtls.length > 0 && pagezone.pagezonedtls[0].image != null) {
					$(pagezoneDiv).find('#rotatable').css('background-image', 'url(/pixsigndata' + pagezone.pagezonedtls[0].image.thumbnail + ')');
				...
				if (pagezone.diy != null) {
					$(pagezoneDiv).find('img').attr('src', '/pixsigndata' + pagezone.diy.thumbnail);
					$(pagezoneDiv).find('img').attr('width', '100%');
					$(pagezoneDiv).find('img').attr('height', '100%');
				}
		...
		function enterPagezoneFocus(pagezone) {
			...
			$('.volumeRange').ionRangeSlider('update', {
				from: pagezone.volume
			});

			$('.intervaltimeRange').ionRangeSlider({
				min: 5,
				max: 60,
				from: 10,
				type: 'single',
			...
			$('.intervaltimeRange').ionRangeSlider('update', {
				from: pagezone.intervaltime
			});
			...
			$('.speedRange').ionRangeSlider({
				...
				hasGrid: false,
				onChange: function(data) {
					_self.Zone.speed = $('input[name=speed]').val();
					refreshPagezone(_self.Zone);
				}
			});
			$('.speedRange').ionRangeSlider('update', {
				from: pagezone.speed
			});
			...
			$('#SubObjectSelect').select2({
				...
				formatResult: function (item) {
					if (item.page != null && item.page.snapshot != null) {
						return '<span><img src="/pixsigndata' + item.page.snapshot + '" height="25" /> ' + item.name + '</span>';
					} else {
		...
		function refreshDiySelect() {
			$('#DiySelect').select2({
				...
				formatResult: function(data) {
					...
					var html = '<span><img src="/pixsigndata' + data.diy.thumbnail + '" width="' + width + 'px" height="' + height + 'px"/> ' + data.diy.name + '</span>';
					return html;
				},
				formatSelection: function(data) {
					var width = 30;
		...
		$('#VideoTable').dataTable({
			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
				...
				videohtml += '<img src="/pixsigndata' + aData.thumbnail + '" class="imgthumb" width="' + thumbwidth + '%" alt="' + aData.name + '" />';
		...
		$('#BGImageTable').dataTable({
			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
				...
				bgimagehtml += '<img src="/pixsigndata' + aData.thumbnail + '" class="imgthumb" width="' + thumbwidth + '%" alt="' + aData.name + '" />';
		...
		function displayBGImagePreview() {
			...
			if (_self.Zone.pagezonedtls.length > 0) {
				...
				if (backgroundimage != '') {
					$('#BGImagePreview').css('background-image', 'url(/pixsigndata' + backgroundimage.thumbnail + ')');
