			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
				...
				for (var i=0; i<aData.subpages.length; i++) {
					...
					if (aData.subpages[i].snapshot != null && aData.subpages[i].snapshot != '') {
						...
						pagehtml += '<img src="/pixsigndata' + aData.subpages[i].snapshot + '?t=' + new Date().getTime() + '" class="imgthumb" width="' + subthumbwidth + '%" />';
					}
					pagehtml += '</div></a>';
		...
		function refreshFromPageTable() {
			...
			for (var i=0; i<_design.Subobjects.length; i++) {
				...
				if (frompage.snapshot != null) {
					...
					fromPageTableHtml += '<img src="/pixsigndata' + frompage.snapshot + '?t=' + new Date().getTime() + '" class="imgthumb" width="' + thumbwidth + '%" alt="' + frompage.name + '" />';
				}
	...
	var initCopyModal = function () {
		...
		$('#PageLibTable').dataTable({
			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
				...
				if (aData.snapshot != null && aData.snapshot != '') {
					var thumbwidth = aData.width > aData.height? 100 : 100*aData.width/aData.height;
					pagelibhtml += '<img src="/pixsigndata' + aData.snapshot + '?t=' + new Date().getTime() + '" class="imgthumb" width="' + thumbwidth + '%" alt="' + aData.name + '" />';
				}
		...
		$('#TouchpageLibTable').dataTable({
			...
			'fnRowCallback': function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
				...
				if (aData.snapshot != null && aData.snapshot != '') {
					var thumbwidth = aData.width > aData.height? 100 : 100*aData.width/aData.height;
					touchpagelibhtml += '<img src="/pixsigndata' + aData.snapshot + '?t=' + new Date().getTime() + '" class="imgthumb" width="' + thumbwidth + '%" alt="' + aData.name + '" />';
				}
				...
				for (var i=0; i<aData.subpages.length; i++) {
					...
					if (aData.subpages[i].snapshot != null && aData.subpages[i].snapshot != '') {
						var thumbwidth = aData.subpages[i].width > aData.subpages[i].height? 100 : 100*aData.subpages[i].width/aData.subpages[i].height;
						touchpagelibhtml += '<img src="/pixsigndata' + aData.subpages[i].snapshot + '?t=' + new Date().getTime() + '" class="imgthumb" width="' + thumbwidth + '%" />';
					}
					touchpagelibhtml += '<div class="mask">';
					touchpagelibhtml += '<div>';
		...
		$('body').on('click', '.pix-copypage-add', function(event) {
			...
			if (ps.length == 0 && selectedPage.pageid != _page.pageid) {
				...
				if (selectedPage.snapshot != null && selectedPage.snapshot != '') {
					thumbhtml = '<div class="thumbs" style="width:40px; height:40px;"><img src="/pixsigndata' + selectedPage.snapshot + '" class="imgthumb" width="100%"></div>';
				}
