...
<%@page import="com.broadvideo.pixsign.domain.Privilege"%>
	...
	<div class="page-header navbar navbar-fixed-top">
		...
		<div class="page-header-inner">
			...
			<div class="page-logo">
				...
				%>
				<img src="/pixsigndata<%=session_org.getLogo()%>" height="40" alt="logo" />
				<%
					} else if (sdomain != null) {
				%>
				<img src="/pixsigndata/sdomain/<%=sdomain.getCode()%>/logo.png?t=1" height="40" alt="logo" />
				<%
					} else {
				%>
				...
				for (Privilege p : pList) {
					...
					if (p.getParent() == null && p.getType().equals("1")) {
						...
						if (p.getMenuurl() != null && p.getMenuurl().length() > 0) {
							...
							out.println("<a href=\"/pixsign/" + subsystem + "/" + p.getMenuurl()+"?CurrentP="+p.getPrivilegeid() + "\">");
							...
							for (Privilege child : p.getChildren()) {
								if (superFlag || myPrivilegeidList.indexOf(child.getPrivilegeid()) >= 0) {
									if (child.getType().equals("1")) {
										...
										if (superFlag || myPrivilegeidList.indexOf(child.getPrivilegeid()) >= 0) {
											out.println("<li class=\"" + liClass + "\">");
											out.println("<a href=\"/pixsign/" + subsystem + "/" + child.getMenuurl()+"?CurrentP="+child.getPrivilegeid()+"&ParentP="+child.getParentid() + "\">" + child.getName() + "</a>");
										} else {
