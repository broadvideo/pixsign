...
var App = function () {
	...
	var handleCustom = function () {
        ...
        //selectpicker初始化
        $.fn.selectpicker.defaults.noneSelectedText="请选择";
    }
	var handleLanguage = function () {
		$('#chinese').click(function(event){
			document.location.href=ctx+"/main.action?locale=zh_CN";						 
		});
		$('#english').click(function(event){
			document.location.href=ctx+"/main.action?locale=en_US"; 
		});
	}
	var sidebarFocus = function () {
		var pagename= $.trim($("ul.breadcrumb li:last").text());
		$(".page-sidebar-menu a").each(function(){
			var menuname=$(this).text();
			if((menuname.indexOf(pagename))>=0){
				$(this).parents("li").addClass("active");
				$(this).has(".title").append("<span class='selected'></span>");
				$(this).parent().parent().prev().has(".title").append("<span class='selected'></span>");
			}
		});
	}
	var handleServerTimeUpdate = function () {
		setTimeout(function(){handleServerTimeUpdate();},1000);
		var curTimeStamp=parseInt(sessionStorage.serverTimeStamp);
		curTimeStamp+=1000;
		sessionStorage.serverTimeStamp=curTimeStamp;
		var d = new Date();
		d.setTime(curTimeStamp);
		var curMonth=d.getMonth()+1;
		//var timeString=d.getFullYear()+"-"+ curMonth +"-"+d.getDate()+" "+d.getHours()+":"+d.getMinutes()+":"+d.getSeconds();
		//var timeString=d.toString();
		var timeString=d.toLocaleString();
		var timeDom='<a style="color:#fff">' + common.view.v11 + timeString + '</a>';
		$("#server-time").html(timeDom);
	}
	var handleServerTime = function () {
		$.ajax({
			url: ctx + '/system/ajaxGetSystemTime.action',
			type: 'GET',
			...
			success: function(result) {
				switch(result.retcode){
					...
					case "1":
						...
						var curTimeZone=curDate.getTimezoneOffset()*60000;
						var curTime=parseInt(result.curtimestamp)+curTimeZone;
						sessionStorage.serverTimeStamp=curTime;
						sessionStorage.TZOffset=result.timezone_offset;
						handleServerTimeUpdate();
						break;
				}
			},
	...
	var handleMultiLanguageFlag = function () {
		if(i18nOpenFlag==0){
			$("a#chinese").hide();
			$("a#english").hide();
		}
    ...
    return {
        ...
        init: function () {
			...
			//Custom handlers, added by elvis
            handleCustom();
			handleLanguage();
			sidebarFocus();
			handleServerTime();
			handleMultiLanguageFlag();
        },
        ...
        dataTableOptions: {
            ...
            processing: true,
            language: {
                "processing": "<img src='" + res + "/assets/img/ajax-loading.gif' />",
