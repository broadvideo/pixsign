...
public class InitService {
	...
	public String init(String request) {
		...
		try {
			...
			if (!org.getOrgtype().equals("2") && org.getBackupmedia() != null) {
				...
				fileElement.addContent(new Element("id").setText("backup" + org.getBackupmedia().getMediaid()));
				if (org.getBackupmedia().getUploadtype().equals("0")) {
					fileElement.addContent(new Element("downloadmode").setText("0"));
					fileElement.addContent(new Element("uri").setText(org.getBackupmedia().getFilename()));
				} else if (org.getBackupmedia().getUploadtype().equals("1")) {
					fileElement.addContent(new Element("downloadmode").setText("2"));
					fileElement.addContent(new Element("uri").setText(org.getBackupmedia().getUri()));
				}
				fileElement.addContent(new Element("name").setText(org.getBackupmedia().getFilename()));
				fileElement.addContent(new Element("size").setText("" + org.getBackupmedia().getSize()));
				fileElement.addContent(new Element("md5").setText(org.getBackupmedia().getMd5()));
			}
			Element configElement = new Element("config");
			rsproot.addContent(configElement);
			configElement.addContent(new Element("serviceurl").setText("http://" + serverip + ":" + serverport
					+ "/pixservice/rest/"));
			configElement.addContent(new Element("staturl").setText("http://" + serverip + ":" + serverport
					+ "/pixservice/rest/"));
			...
			configElement.addContent(new Element("videodownloadserver").setText("http://" + serverip + ":" + serverport
					+ "/video/"));
			configElement.addContent(new Element("imgdownloadserver").setText("http://" + serverip + ":" + serverport
					+ "/image/"));
			configElement.addContent(new Element("xlfdownloadserver").setText("http://" + serverip + ":" + serverport
					+ "/pixorg/web/org/layout!download.action?layoutid="));
			if (org.getBackupmedia() != null) {
				configElement.addContent(new Element("backupfileid").setText("backup" + org.getBackupmediaid()));
			}
